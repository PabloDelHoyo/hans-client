...
class PositionCodec:
    ...
    def __init__(self, num_answers: int, radius=430):
        ...
        angles = np.linspace(-np.pi / 2, -np.pi / 2 + 2 * np.pi, num_answers, endpoint=False)

        # We truncate because that is what is done in the original
        # (num_answers, 2)
        self.answer_points = (
            np.trunc(radius * np.stack((np.cos(angles), np.sin(angles)), axis=1))
        )
    ...
    def encode(self, point: np.array) -> np.array:
        ...
        sort_indices = np.argsort(self.distance_to_answers(point))[:2]
        new_basis = self.answer_points[sort_indices]
        point_new_basis = np.linalg.solve(new_basis.T, point)

        encoded_position = np.zeros(self.answer_points.shape[0])
        encoded_position[sort_indices] = point_new_basis
